...
const SUPPORTED_GAME_TYPES = ["bullet", "blitz", "rapid", "classical"];
    ...
    const actions = (message) => ({
      GET_LICHESS_ACCESS_TOKEN: () => {
        if (message.payload) {
          ...
          // initSubTabs(port);
          fetchOpponentNotes();
          fetchUserAnalytics(message.payload.value);
        }
      },
      AUTH_LICHESS: () => {
        setAccessToken(message.payload.value);
      ...
      {accessToken ? (
          <div className="ca_container">
            <div className="ca_section ca_opponent_info">
              <div className="ca_elo_range">
                <div className="ca_elo_range_lowest">
                  <div className={`ca_elo_range_lowest_value ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`}>????
                  </div>
                </div>
                <div className="ca_elo_range_current">
                  <div className={`ca_elo_range_current_value ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`}>????
                  </div>
                </div>
                <div className="ca_elo_range_highest">
                  <div className={`ca_elo_range_highest_value ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`}>????
                  </div>
                </div>
              </div>
              <div style={{ display: "flex", alignItems: "center", fontSize: "12px" }}>
                <div className="ca_opponent_info_section" title="Puzzle Rating">
                  ...
                  <span className={`ca_puzzle_rating ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`}>????</span>
                </div>
                <div className="ca_opponent_info_section" title="Disconnects">
                  ...
                  <span className={`ca_disconnects ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`}>????</span>
                </div>
                <div className="ca_opponent_info_section" title="Streak">
                  <span data-icon=""></span>
                  <span className={`ca_win_streak_value ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`}>???</span>
                </div>
              </div>
            </div>
            <div className="ca_section ca_tabs">
              ...
              <span className={`ca_tab ca_tab_icon ca_notes_tab_trigger ${opponentNotes ? "ca_green_colour" : ""}`}>
            <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
            ...
            <div className="ca_section ca_tab_section ca_stats" style={{ margin: 0 }}>
              <canvas id="ca_stats_chart" className={`ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`} height="90"></canvas>
              <canvas id="ca_stats_move_times_chart" className={`ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`} height="110"></canvas>
            </div>
            <div className="ca_section ca_tab_section ca_openings ca_hidden" style={{ margin: 0 }}>
              <canvas id="ca_openings_chart" className={`ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`} height="200"></canvas>
            </div>
            <div className="ca_section ca_tab_section ca_notes ca_hidden" style={{ margin: 0 }}>
              <form action="" id="ca_save_opponent_notes_form" onSubmit={onSubmitSaveOpponentNotes}>
                <div style={{ marginBottom: "10px" }}>
                  <textarea id="ca_opponent_notes" className={`ca_textarea ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`} value={opponentNotes ? opponentNotes : ""} onChange={e => setOpponentNotes(e.target.value)}></textarea>
                </div>
                <div>
                  <button type="submit" className={`ca_button ca_btn-win ca_placeholder ${userAnalytics ? "" : "ca_placeholder_enabled"}`}>
                    Save
                  </button>
                </div>
              </form>
          ...
          <div className="ca_auth_container">
            ...
            <div className="ca_auth_content_container">
              ...
              <div>
                ...
                <p style={{marginBottom: 0}}>You will be prompted to
                  re-authorise when the access token expires.</p>
              </div>
            </div>
            <button id="auth_lichess_btn"
                    className="ca_button ca_button_large ca_btn-win"
                    onClick={onClickAuthoriseWithLichess}>
